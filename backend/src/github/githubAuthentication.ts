...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    if (!user) {
      userId = await createGraphcoolUser(api, githubUser.name, githubUser.avatar_url, githubUser.id)
    } else {
      userId = user.id
    }
...
async function createGraphcoolUser(api: GraphQLClient, name: string, avatarUrl: string, githubUserId: string): Promise<string> {
  const mutation = `
    mutation createUser($name: String!, $avatarUrl: String, $githubUserId: String!) {
      createUser(
        name: $name,
        avatarUrl: $avatarUrl,
        githubUserId: $githubUserId
